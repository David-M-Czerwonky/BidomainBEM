function [Efield,t2p,p,epseff,xval,iter,resvec,rhs]=adjointBEM_TMS(t2p,p,epseff,rs,js,ro)
...
nt=numel(t2p)/3;np=numel(p)/3;nc=numel(rs)/3;
...
x(end+1)=-sum(area(1:end-1).*x(:))/sum(area(end));
chspace=Ach'*x;
iprec=[0 0];
