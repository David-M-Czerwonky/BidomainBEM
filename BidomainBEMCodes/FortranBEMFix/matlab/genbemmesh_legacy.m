...
nte=numel(te2p)/4;
np=numel(p)/3;np2=0;
mex_id_ = 'o int = countfaceslegacy(i int, i int[xx], i double[x], io int[x])';
[nf, np2] = BEM(mex_id_, nte, te2p, condu, np2, 4, nte, nte, 1);
t2p=zeros([3,nf]);
p2=zeros([3,np2]);
epseff=zeros([nf,1]);
mex_id_ = 'makeBEMmeshlegacy(i int, i int[xx], i double[xx], i double[x], io int[xx], io double[xx], io double[x])';
