...
recursive subroutine groupcreatorMLct(rs,grs,ns,ro,gro,no,del,ct,del2)
implicit none
...
real(kind=dp),intent(in),dimension(:) :: del2
!internals
...
call groupcreatorMLct(rs(:,keeps(1:cts)),grs(keeps(1:cts)),cts, &
    ro(:,keepo(1:cto)),gro(keepo(1:cto)),cto,del,ct,del2)
end if
end do
...
do oloo=1,no
do sloo=1,ns
if (sqrt((ro(1,oloo)-rs(1,sloo))**2 + &
         (ro(2,oloo)-rs(2,sloo))**2 + &
         (ro(3,oloo)-rs(3,sloo))**2) < max(del2(oloo),del2(sloo)) ) then
    ct=ct+1
end if
end do
end do
...
recursive subroutine groupcreatorML(rs,grs,ns,ro,gro,no,del,ct,col,row,ncol,del2)
implicit none
...
call groupcreatorML(rs(:,keeps(1:cts)),grs(keeps(1:cts)),cts, &
    ro(:,keepo(1:cto)),gro(keepo(1:cto)),cto,del,ct,col,row,ncol,del2)
end if
end do
...
subroutine generategroup(t2p,nt,p,del,col,row,ncol,del2)
implicit none
integer(kind=spint),intent(in) :: nt
...
integer(kind=spint),dimension(:),allocatable :: colt,rowt,grs
...
ct=0
ct2=0
call countneighbors(t2p,nt,col,row,ct,ct2)
!figure out how many nearfield 
call groupcreatorMLct(rs,grs,ns,rs,grs,ns,del,ct,del2)	  
	  ncol=ct+ct2
!write actual array	  
allocate(colt(ncol))
allocate(rowt(ncol))
!write neighbors
ct=0
call writeneighbors(t2p,nt,col,row,colt,rowt,ncol,ct,ct2)
!write nearfield
call groupcreatorML(rs,grs,ns,rs,grs,ns,del,ct,colt,rowt,ncol,del2)
